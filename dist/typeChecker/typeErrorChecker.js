    ...
    switch (typeNode.type) {
        ...
        case 'TSTypeReference':
            ...
            if (typeNode.typeParameters) {
                ...
                return (0, utils_1.tVar)(name, typesToSub);
            }
            return (0, utils_1.tVar)(name);
...
function getHeadType(node, type) {
    switch (type.kind) {
        ...
        case 'union':
            return (0, utils_1.tUnion)(...type.types.map(type => getHeadType(node, type)));
        case 'variable':
    ...
    switch (type.kind) {
        ...
        case 'list':
            return (0, utils_1.tList)(type.elementType, type.typeAsPair && type.typeAsPair.tailType.kind === 'pair'
                ...
                : undefined);
        case 'union':
            return (0, utils_1.tUnion)(...type.types.map(type => getTailType(node, type)));
        case 'variable':
...
function lookupTypeAliasAndRemoveForAllTypes(node, varType) {
    ...
    const aliasType = (0, utils_1.lookupTypeAlias)(varType.name, env);
    if (!aliasType) {
